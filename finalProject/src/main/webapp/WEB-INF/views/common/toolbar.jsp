	...
	<div style="height: 30px; background: lemonchiffon;">
			 ...
			 function TintsetImg(value) {
				...
				$.ajax({
							...
							success : function(data) {
								...
								$("#insertField").focus();
								document.execCommand('insertHTML',false,"<img src='/evening/resources/usedTextImgs/"+data.imgName+"' name="+data.imgName+" id='textImgs'>");
								$("#insertField").focus();
		...
		<script>
			...
			$('html').click(function (e) {
				...
				// if (!$(e.target).hasClass('rgbClick') && !$(e.target).hasClass('btn-C-1')) {
				//     $('.font-C').removeClass('on');
				// }
			...
			// $('.inFontTag').on("click",function(){
			//     $(this).toggleClass('on');
			// }    

			// window.onclick = function (event) {
			//     if (event.target != document.getElementsByClassName('inFontTag')) {
			//         $('.inFontTag').removeClass('on');
			//     }
			// }
